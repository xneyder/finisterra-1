def aws_service_discovery_http_namespace(self):
    ...
    paginator = self.sd_client.get_paginator("list_namespaces")
    for page in paginator.paginate():
        for namespace in page["Namespaces"]:
            if namespace["Type"] == "HTTP":
                namespace_id = namespace["Id"]
                http_namespace = self.sd_client.get_namespace(Id=namespace_id)[
                    "Namespace"]
                print(
...
def aws_service_discovery_instance(self):
    ...
    for page in paginator.paginate():
        for service in page["Services"]:
            ...
            instance_paginator = self.sd_client.get_paginator("list_instances")
    ...
    for page in paginator.paginate():
        for namespace in page["Namespaces"]:
            if namespace["Type"] == "DNS_PRIVATE":
                namespace_id = namespace["Id"]
                private_dns_namespace = self.sd_client.get_namespace(Id=namespace_id)[
                    "Namespace"]
                print(
    ...
    for page in paginator.paginate():
        for namespace in page["Namespaces"]:
            if namespace["Type"] == "DNS_PUBLIC":
                namespace_id = namespace["Id"]
                public_dns_namespace = self.sd_client.get_namespace(Id=namespace_id)[
                    "Namespace"]
                print(
...
def aws_service_discovery_service(self):
    ...
    for page in paginator.paginate():
        for service in page["Services"]:
            service_id = service["Id"]
            sd_service = self.sd_client.get_service(Id=service_id)["Service"]
            print(f"  Processing AWS Service Discovery Service: {service_id}")
